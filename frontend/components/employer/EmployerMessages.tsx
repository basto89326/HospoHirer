...
import { mockEmployerConversations, type Conversation } from "@/lib/mockData";

export default function EmployerMessages() {
  const [conversations, setConversations] = useState(mockEmployerConversations);
          ...
          <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-5 py-5 space-y-3 bg-gray-50/40">
            {selected.messages.map((msg) => (
              ...
              >
                ...
                >
                  <p>{msg.text}</p>
                  <p className="text-[10px] mt-1 text-gray-400">{msg.sentAt}</p>
                </div>
              </div>
            ))}
          </div>
